...
class Instellation(DiagnosticComponent):
    ...
    def __init__(self, diurnal_average=False, **kwargs):
        self._diurnal_average = diurnal_average
    ...
    def array_call(self, state):
        ...
        zen_angle = sun_zenith_angle(state['time'], lon=lon_radians, lat=lat_radians, diurnal_average=self._diurnal_average)
...
def star_zenith_azimuth(model_time, lon, lat, diurnal_average=False):
    """
    ...
    if diurnal_average:
        hour_angle_range = np.linspace(0, 2*np.pi, 100)  # assume that sidereal days are 24 hours
        zenith_angle_range = np.arccos(np.sin(lat) * np.sin(dec) +
                                       np.cos(lat) * np.cos(dec) * np.cos(hour_angle_range))
        zenith_angle_range[zenith_angle_range>np.pi/2] = np.pi/2
        zenith_angle_range[zenith_angle_range<-np.pi / 2] = -np.pi/2
        zenith_diurnal_average = (1/(2*np.pi))*integrate.trapz(zenith_angle_range, hour_angle_range)
        zenith = np.array(zenith_diurnal_average)

    return zenith, azimuth


def sun_zenith_angle(model_time, lon, lat, diurnal_average=False):
    """
    Sun-zenith angle for lon, lat at model_time.
    lon,lat in radians.
    The angle returned is in radians
    """
    zenith, azimuth = star_zenith_azimuth(model_time, lon, lat, diurnal_average)
    return zenith
